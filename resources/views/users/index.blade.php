<x-app-layout>
    <div class="mx-4 md:mx-12 my-4 p-4 border border-gray-400 rounded-lg">
        <form action="{{ route('users.index') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-center">
            ...
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg transition-all w-full md:w-auto">
                ค้นหา
            </button>

            @if(request('search') || request('role'))
                <a href="{{ route('users.index') }}" class="text-gray-500 hover:text-red-500 underline text-sm whitespace-nowrap">
                    ล้างค่าค้นหา
                </a>
            @endif
